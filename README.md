...
当前版本见 `package.json` 中的 `version` 字段（例如 `2.5.6`）。

- **标签打印** — 小标签/大标签，带二维码；含**普通标签**、**打包标签**、**双语大标签**（中英描述，可与配货单联动）
- **入库 / 出库管理** — 搜索产品并录入数量和备注，支持 Excel 批量导入入库/出库
- **配货单管理** — 按单号查询、拼箱打印（普通/双语）、统一出库、导出送货单，支持 Excel 批量导入/导出配货单
- **拼送货单** — 将多个配货单条目合并整理后导出送货单等（与配货数据联动）
- **实时库存查看与修改** — 表格视图外，支持**仪表盘**、**库存趋势**、**配单用量**等视图；支持手动调整与 Excel 批量导入初始库存
- **打印与库存** — 打印页可勾选「不计入库存」；取消勾选后，打印将按数量扣减库存
- **操作记录审计** — 记录每一笔操作的人员和时间，支持撤销单条记录
- **用户权限管理** — 支持管理员和普通用户角色，可独立控制库存查看权限和数据管理权限
- **物料库管理** — 产品数据的增删改查，支持中英文描述、**多张图纸**附件管理，支持从 Excel（.xlsx / .xls / .csv）批量导入（需数据管理权限）
...
（`dev` 脚本经 `scripts/dev.js` 调用 `electron-vite`，避免部分环境下 `ELECTRON_RUN_AS_NODE` 导致 Electron 误判。）
...
系统已提供局域网只读开放接口，供 OpenClaw/Qclaw/Accio Work 等智能体使用：

- 前缀：`/api/open/v1/*`
- 鉴权：`Authorization: Bearer <OPEN_API_TOKEN>`，或请求头 `x-open-api-token: <OPEN_API_TOKEN>`
- 仅支持 `GET`（写操作请求会被拒绝）
- 默认限流：每 IP 每分钟最多约 180 次请求
- 提供 `GET /api/open/v1/openapi.json` 供工具发现接口
- 访问会写入审计日志；管理员可通过 `GET /api/auth/open-api-audit-logs?limit=200` 查看
- 详细文档：`docs/open-api-guide.md`

云端化演进方向（规划稿，非当前实现）：`docs/cloud-phase2-plan.md`

可通过环境变量控制：

- `SERVER_HOST`：服务监听地址（默认 `0.0.0.0`）
- `SERVER_PORT`：服务端口（默认 `3456`）
- `OPEN_API_ENABLED`：开放接口总开关（`1` / `0`）
- `OPEN_API_TOKEN`：开放接口固定 token（可选，未设置时使用系统内生成 token）
...
侧边栏提供三个入口，均基于同一套打印流程，模板与字段略有区别：

1. **标签打印**：常规小/大标签，带单号、工程名等（与配货单「拼箱」跳转一致）
2. **打包标签**：面向打包场景的版式与选项
3. **双语标签**：大标签上展示中英文描述；配货单支持「打开双语标签」预填行内中英文
...
3. 从搜索结果中选择要打印的物品
4. 设置物品数量、打印张数、模板（小标签 / 大标签）及版式相关选项
5. 右侧预览后点击打印

页面提供「**不计入库存（勾选后打印不扣减库存）**」选项，**默认勾选**（即默认打印不扣减）。取消勾选后，打印将按界面提示扣减库存；扣减规则与按钮文案一致。系统允许负库存，与出库逻辑一致。
...
用于把多个配货单中的物料行汇总到一张**拼送货单** Excel（送货单版式，默认一式两份）。

1. 进入「拼送货单」页面，点击「**增加物料**」
2. 在弹窗中输入订单号 →「**查询订单**」，勾选需要的行 →「**确认增加**」（可多次添加不同订单）
3. 在主列表中可勾选后「**移除所选**」调整清单
4. 点击「**导出拼送货单**」下载 `.xlsx`；导出成功后会尝试将相关行标记为已打送货单（若接口失败会有提示，文件仍可能已下载）
...
1. 进入「库存」页面
2. 顶部可在**表格 / 仪表盘 / 趋势 / 配单用量**等视图间切换，便于总览与按单分析
3. 在表格模式下可搜索过滤，查看所有产品的实时库存数量
4. 点击「修改」可手动调整库存数量（需填写修改原因）
5. 支持通过「批量导入库存」从 Excel 导入数量
6. 每次修改都会记录操作日志
...
1. 进入「物料库」页面
2. 可进行以下操作：
  - **新增物品**：手动填写物料信息（含中文/英文描述等）
  - **编辑物品**：修改已有物品信息
  - **图纸**：为单个物料维护多张图纸附件（查看/上传/排序）
  - **删除物品**：单个或批量删除
  - **导入 Excel**：批量导入物品数据（支持英文描述列，见下表）
  - **清空全部**：清空所有物品数据（仅管理员）
...
| 物品名称 / 名称 / name                            | 必填   |
| 物料描述 / 描述                                   | 选填   |
| 英文描述 / 英文 / description_en / Description EN | 选填   |
| 规格 / spec                                   | 选填   |
| 等级 / grade                                  | 选填   |
| 表面处理                                        | 选填   |
| 材质                                          | 选填   |
...
| 条形码     | JsBarcode                         |
| 二维码     | qrcode                            |
| Excel   | SheetJS (xlsx) + ExcelJS（导出/复杂表格） |
| 图表      | Recharts                          |
...
```bash
chmod +x dist/库存管理系统-<版本号>.AppImage
./dist/库存管理系统-<版本号>.AppImage
```

（`<版本号>` 与 `package.json` 中 `version` 一致，例如 `2.5.6`。）
...
│   ├── main/                   # Electron 主进程
│   │   ├── index.ts            # 应用入口，创建窗口
│   │   ├── ipc-handlers.ts     # IPC（如系统打印）
│   │   ├── server.ts           # Express HTTP 服务器（REST API、静态文件、开放接口）
│   │   ├── api-routes.ts       # 业务路由（产品、库存、配货单、用户、认证等）
│   │   ├── auth.ts             # JWT 认证与权限中间件
│   │   ├── database.ts         # SQLite 封装
│   │   └── assistant/          # 内置分析助手相关逻辑（后端模块，随版本迭代）
│   ├── preload/                # 预加载脚本
...
│           ├── pages/
│           │   ├── LoginPage.tsx              # 登录页
│           │   ├── PrintPage.tsx             # 标签打印（普通/打包/双语）
│           │   ├── StockInPage.tsx           # 入库页
│           │   ├── StockOutPage.tsx         # 出库页
│           │   ├── StockPage.tsx             # 库存（多视图）
│           │   ├── PickingOrderPage.tsx      # 配货单
│           │   ├── MergedDeliverySheetPage.tsx # 拼送货单
│           │   ├── DataPage.tsx               # 物料库
│           │   ├── UserManagePage.tsx       # 用户管理
│           │   ├── AssistantPage.tsx        # 助手 UI（随功能启用）
│           │   └── AssistantSettingsPage.tsx
│           ├── components/
│           │   ├── LabelLarge.tsx / LabelLargeBilingual.tsx
│           │   ├── LabelSmall.tsx
│           │   ├── ProductForm.tsx / ProductDrawingsModal.tsx
│           │   ├── ImportModal.tsx / StockImportModal.tsx / ImportInventoryModal.tsx
│           │   ├── StockDashboardPanel.tsx / StockTrendPanel.tsx / StockOrderUsagePanel.tsx
│           │   └── ResizableTable.tsx 等
│           ├── styles/
│           │   ├── global.css       # 全局样式
...
- `**pages/*Page.tsx**`：对应侧边栏各功能入口（登录、多种标签打印、入库、出库、配货单、拼送货单、库存、物料库、用户管理等），只聚焦页面业务逻辑。
...
- 主要数据表包括：`app_meta`、`products`、`product_drawings`、`inventory`、`inventory_logs`、`users`、`picking_order_items`、`picking_order_splits`、`print_logs`、`open_api_audit_logs` 等（随迁移可能增加列或表）
